...
%
%    [TODO: check and update documentation; some terms missing above]
...
if isfield(pr, 'kappa_prior') && pr.kappa_prior
    ...
    si.gamma_f_p_u_prior.bernoulli_p_nat = log(pr.p_u) - log1p(-pr.p_u);
...
%     si.tau2.normal_mu(update_inds) = (1 - op.damp) * si.tau2.normal_mu(update_inds) + op.damp * (ti.tau2.normal_mean(update_inds) ./ ti.tau2.normal_var(update_inds) - ca.tau2.normal_mu(update_inds));
    % TODO: need to worry about negative cavities and/or negative site
    % variances for these also?
    update_inds = update_inds & ca.tau2.normal_tau(:) > 0;
    
    new_tau_tau2_site = 1 ./ ti.tau2.normal_var - ca.tau2.normal_tau;
    switch op.robust_updates
        case 0
        case 1
            inds_tmp = new_tau_tau2_site(:) > 0;
            update_inds = update_inds & inds_tmp;
        case 2
            inds = new_tau_tau2_site(:) <= 0;
            new_tau_tau2_site(inds) = op.min_site_prec;
            ti.tau2.normal_var(inds) = 1./(op.min_site_prec + ca.tau2.normal_tau(inds));
    end
    new_mu_tau2_site = ti.tau2.normal_mean ./ ti.tau2.normal_var - ca.tau2.normal_mu;
    
    si.tau2.normal_tau(update_inds) = (1 - op.damp) * si.tau2.normal_tau(update_inds) + op.damp * new_tau_tau2_site(update_inds);
    si.tau2.normal_mu(update_inds) = (1 - op.damp) * si.tau2.normal_mu(update_inds) + op.damp * new_mu_tau2_site(update_inds);
end
